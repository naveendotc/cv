...
\ifCommonmaincls
\begin{Example}
  \phantomsection\label{desc:\csname label@base\endcsname.option.cleardoublepage.example}%
...
Option \Option{twoside} in \autoref{sec:typearea.typearea},
...
\autoref{cha:scrlayer-scrpage}) verwendet werden.
...
\IfNotCommon{scrlttr2}{%
  ...
  \IfCommon{maincls}{% Umbruchoptimierungsalternative
...
\IfCommon{maincls}{ist ebenso wie die Standard-Anweisung
  \Macro{cleardoublepage} von der in \autoref{sec:maincls.structure},
  \autopageref{desc:maincls.option.open} erklärten Option
  \Option{open}\IndexOption{open}\important{\Option{open}} abhängig und
  entspricht je nach Einstellung einer der in den vorherigen Absätzen
  erläuterten Anweisungen.

}%
\IfNotCommon{maincls}{entspricht \IfCommon{scrlttr2}{bei der Klasse
    \Class{scrlttr2}}\IfCommon{scrextend}{beim Paket \Package{scrextend}}
  ebenso wie die Standard-Anweisung \Macro{cleardoublepage} den
  entsprechenden, zuvor erklärten Anweisungen\IfNotCommon{scrlttr2}{ für den
    Umbruch zur nächsten ungeraden Seite}.}  \IfCommon{scrlttr2}{%
...
\end{lstcode}
  ...
  \IfNotCommon{scrextend}{im Dokument }% Umbruchoptimierung!!!
  schreiben Sie nun:
\begin{lstcode}
